...
<ComparisonTable
  title='vs Built-in Chrome CDP (--remote-debugging-port)'
  headers={['', 'Built-in CDP', 'Playwriter']}
  rows={[
    ['Setup', 'Restart Chrome with special flags', 'Click extension icon'],
    ['Confirmation dialog', 'Automation infobar agents can\'t dismiss', 'No blocking dialog'],
