...
description: How Playwriter protects local automation and high-trust remote tab sharing with consent, encryption, and revocation.
icon: lucide:shield
---
...
│  └─────────────┘          └──────────────────┘          └──────────────┘                 │
│                                                                                          │
│  Nothing leaves localhost unless you explicitly enable a remote feature                  │
└──────────────────────────────────────────────────────────────────────────────────────────┘
```

## Security guarantees

**Local by default.** The WebSocket server binds to localhost. Traffic leaves your machine only when you confirm [Remote control](./remote-access.mdx) for a tab or configure full relay access with a tunnel and auth token.
...
When using one-tab **Remote control** (see the [full Remote control security page](./remote-control.mdx)):

- **Explicit consent.** A short confirmation, with a Read more link, explains that the recipient can view and control the tab before the tunnel starts.
- **Not a sandbox.** The selected tab is the starting control surface, but the recipient receives broad CDP access and must be fully trusted. A short denylist blocks new-tab creation, explicit whole-profile cookie APIs, and obvious destructive clears.
...
- **Revocable bearer link.** Anyone with the secret URL can connect until the user opens the **Remote ON** dropdown and clicks **Stop sharing**. Treat the URL like a password.
...
- **Privacy disclosure.** Remote traffic can contain screenshots, page content, URLs, input events, network data, cookies, authentication data, and browser storage. Read the [privacy policy](../privacy.mdx) before sharing sensitive tabs.

When exposing the full relay through [remote access](./remote-access.mdx):
