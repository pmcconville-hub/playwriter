...
When using one-tab **Remote control**:

- **Explicit consent.** A native confirmation explains that the agent can read and control the shared tab before the tunnel starts.
...
- **Revocable bearer link.** Anyone with the secret URL can connect until the user clicks **Remote control** again. Treat the URL like a password.
