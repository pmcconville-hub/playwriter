...
- **Scoped access.** The agent receives only the selected tab and popups it opens. Browser-wide destructive commands and new-tab creation are blocked.
...
- **Secret stays in the fragment.** Shared links are `https://playwriter.dev/remote-control#{id}`. A URL fragment is never sent to a server, so the id stays out of request paths, access logs, and Referer headers.
- **Nothing is stored.** Tunnel frames are relayed in memory only, and response caching is off for these tunnels.
