...
The agent controls **only the shared tab** (plus popups it opens, so OAuth and payment flows keep working). Creating new tabs and browser-wide commands like clearing all cookies are blocked with helpful errors. Everything happens visibly in your browser.
...
<Aside>
<Warning>
The URL grants control of your tab **as you** — the agent can navigate the tab to any site where you are logged in. Never share it with anyone you don't trust, and never post it in public channels. Revoke it with the button when the agent is done. The link also dies when you close your browser.
</Warning>
</Aside>

Remote traffic can include **screenshots, page content, URLs, input events, network data, and page-accessible cookies or browser storage**. Payloads are routed in memory and are not stored by Playwriter. Read the [privacy policy](../privacy.mdx) for the complete disclosure.
