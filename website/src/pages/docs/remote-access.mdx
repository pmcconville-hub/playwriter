...
The tunnel id lives in the **URL fragment** (`#xxx`). Fragments are never sent to a server, so the secret stays out of request paths, access logs, and Referer headers.
...
If the stream ever does stall, for example when the whole window is minimised, the viewer automatically falls back to polling screenshots and shows **Low frame rate**. Clicking and typing keep working either way.
</Note>
</Aside>
