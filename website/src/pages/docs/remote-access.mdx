...
## Remote control: share one tab, zero setup

The simplest way to give a remote agent access to your browser is the **Remote control** button (light-blue cloud) in the Playwriter toolbar. It requires **no playwriter install** on your machine — only the extension. The extension itself opens a secret tunnel and exposes the tab behind an unguessable URL.

1. Click the **Remote control** button and confirm that the agent may read and control the tab. [Read more about security](./remote-control.mdx).
2. A prompt containing the secret URL is copied to your clipboard. Paste it to the agent.
3. The agent runs, on its own machine:
...
4. Open the **Remote ON** dropdown and click **Stop sharing** anytime to **revoke access**. The URL dies instantly and is never reused.

The shared tab is the **starting control surface**, not a security sandbox. Remote CDP access is powerful, so the recipient must be fully trusted. A short denylist blocks new-tab creation, explicit whole-profile cookie APIs, and obvious destructive clears.

### Watch and click from any browser

The same link is also a page. Open it and you get a **live view of the tab**, streamed frame by frame, with a URL bar and a **Take control** button for clicking, scrolling, and typing.

That means you can share a tab with a person, not only an agent, and no install is needed on either side.

The tunnel id starts in the **URL fragment** (`#xxx`), so the initial viewer-page request and Referer omit it. The viewer JavaScript then uses the id to connect to the tunnel hostname, which the tunnel service and Cloudflare necessarily process.

<Aside>
<Note>
The stream **keeps running when the sharer switches tabs**. Playwriter holds a debugger attachment on the shared tab, which stops Chrome from backgrounding it, so frames keep arriving at the same rate.
</Note>
</Aside>

<Aside>
<Warning>
The URL grants **broad browser automation access**. A malicious recipient may use CDP capabilities beyond the visible page. Never share it with anyone you do not fully trust, and never post it in public channels. Revoke it with the button when the agent is done. The link also dies when you close your browser.
</Warning>
</Aside>

Remote traffic can include **screenshots, page content, URLs, input events, network data, cookies, authentication data, and browser storage**. Payloads are routed in memory and are not stored by Playwriter. Read the [privacy policy](../privacy.mdx) for the complete disclosure.

Typical use case: you are logged into a website and want an agent (Devin, a cloud bot, a teammate's CLI agent) to do work in your authenticated session without ever sharing a password.

The rest of this page covers the full-relay setup: exposing your **entire relay server** with `playwriter serve` + traforo, which gives a remote machine access to all controlled tabs and requires playwriter installed on the host.
