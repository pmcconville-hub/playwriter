...
The stream captures the session's **`state.page`**. Set it and navigate first, then start:

```bash
playwriter -s 1 -e "state.page = await context.newPage(); await state.page.goto('https://my-generated-content.example')"
