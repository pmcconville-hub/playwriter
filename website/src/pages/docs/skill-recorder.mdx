...
title: 'Record workflows as agent skills'
sidebarTitle: Skill Recorder
description: Record a browser workflow once by hand. Your agent turns it into a reusable skill with verified locators and an importable utils script.
...
```bash
playwriter -s 1 -e 'const { submitProduct } = await import("./.agents/skills/submit-to-directory/submit.js"); state.page ??= await context.newPage(); await submitProduct({ page: state.page, name: "Acme", url: "https://acme.com" })'
```
...
For an SDK that other code imports (scripts, bots, cron jobs), ask for a
**standalone `sdk.ts`**. It uses `playwriter` as a library through the
[Node API](/docs/sessions#node-api) and returns typed data:

```ts
import { connectViaExtension } from 'playwriter'

export async function listProducts(): Promise<{ id: string; name: string }[]> {
