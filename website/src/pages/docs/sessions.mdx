...
Tabs a session creates are grouped in a Chrome **tab group** named `playwriter` by default. Give each session its own group name so its tabs stay visually separate.
