...
`tabGroupColor` is typed as Chrome's tab group colors: `grey`, `blue`, `red`, `yellow`, `green`, `pink`, `purple`, `cyan`, `orange`. `close()` closes leftover pages, disconnects CDP, and deletes the session.
