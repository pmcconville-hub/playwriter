...
playwriter -s 1 -e "state.page.evaluate(() => performance.getEntriesByType('navigation'))"
playwriter -s 1 -e "state.cdp = getCDPSession({ page: state.page }); state.cdp.send('Network.enable')"
```
